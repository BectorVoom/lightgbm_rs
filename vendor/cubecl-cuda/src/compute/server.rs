...
impl CudaServer {
    ...
    fn command<'a>(
        ...
        // CUDA-graph scaffolding: on the SERVER THREAD, record the stream/context and act
        // on any armed begin/end capture command (stream capture is thread-scoped, so it
        // must be started/stopped here, in-order with the launches this thread issues).
        // SAFETY: on the server thread the context is current and the stream is valid.
        unsafe { crate::compute::capture::server_poll(streams.current().sys, self.ctx.context) };
