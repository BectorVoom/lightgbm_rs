...
/// Phase-0 CUDA-graph capture/replay scaffolding (lightgbm_rs perf fork).
/// See [`compute::capture`]. Exposed so a benchmark can capture cubecl's own launch
/// stream and replay it via `cuGraphLaunch`.
pub use compute::capture;
