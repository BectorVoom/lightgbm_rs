...
/// The pinned pip-`lightgbm` version for the Phase-7 W10 advanced-constraints
/// (ADV-01..05) oracle (plan 07-11). Reuses the SAME prebuilt-wheel binary;
/// `constraints-oracle-capture` asserts the installed version BEFORE training so a
/// wrong version can never silently emit divergent constraint goldens
/// (threat T-07-11-SC2).
pub const CONSTRAINTS_ORACLE_LIGHTGBM_VERSION: &str = MODEL_LIGHTGBM_VERSION;

/// The recorded train seed for the Phase-7 W10 constraints oracle (plan 07-11).
/// The SAME seed the learner oracle uses; combined with `deterministic=true
/// force_row_wise=true num_threads=1` it makes the constraint goldens (incl. the
/// extra-trees RNG-replay) byte-idempotent. Recorded in REFERENCE_MANIFEST.md.
pub const CONSTRAINTS_ORACLE_SEED: i32 = LEARNER_ORACLE_SEED;
...
/// `constraints-oracle-capture` (plan 07-11, ADV-01..05) — REAL lib_lightgbm 4.6
/// ADVANCED-CONSTRAINTS learner-oracle capture. Trains single-tree regression
/// models on a shared deterministic numeric corpus across the per-axis matrix
/// (monotone method×penalty×vector, interaction groups, forced single/nested,
/// extra-trees extra_seed RNG-replay, CEGB tradeoff×penalty) on the real prebuilt
/// `lib_lightgbm` and dumps each model's authoritative v4 model text PLUS a JSON
/// sidecar (per-row bins + bin_upper_bound + grad + the constraint axis) under
/// `crates/oracle-harness/tests/fixtures/constraints/`. The pip `lightgbm` is a
/// CAPTURE-time tool only (version-asserted, never a crate dep, never read at
/// `cargo test` time). NEVER `git add` the `LightGBM/` tree.
fn constraints_oracle_capture() -> Result<()> {
    let root = workspace_root()?;
    let python = resolve_capture_python()?;
    let script = root.join("xtask/py/constraints_oracle_capture.py");
    if !script.is_file() {
        bail!("capture script {} not found", script.display());
    }

    let out_dir = root.join("crates/oracle-harness/tests/fixtures/constraints");
    std::fs::create_dir_all(&out_dir)
        .with_context(|| format!("creating fixtures dir {}", out_dir.display()))?;

    eprintln!(
        "xtask constraints-oracle-capture: using python {} (lightgbm {} expected) ...",
        python.display(),
        CONSTRAINTS_ORACLE_LIGHTGBM_VERSION
    );
    run(
        Command::new(&python).arg("-c").arg(format!(
            "import lightgbm,sys; \
             assert lightgbm.__version__=='{ver}', \
             'lightgbm '+lightgbm.__version__+' != recorded {ver}'",
            ver = CONSTRAINTS_ORACLE_LIGHTGBM_VERSION
        )),
        "lightgbm version check",
    )
    .context(
    ...
    eprintln!(
        "xtask constraints-oracle-capture: training the monotone / interaction / \
         forced / extra-trees / CEGB per-axis corpora on real lib_lightgbm ..."
    );
    run(
        Command::new(&python)
            .arg(&script)
            .arg(&out_dir)
            .arg(CONSTRAINTS_ORACLE_SEED.to_string())
            .arg(CONSTRAINTS_ORACLE_LIGHTGBM_VERSION),
        "constraints_oracle_capture.py",
    )?;

    eprintln!(
        "xtask constraints-oracle-capture: done. Wrote per-axis constraint goldens \
         + sidecars under {}.",
        out_dir.display()
    );
    eprintln!(
        "Re-run `cargo run -p xtask -- constraints-oracle-capture` and confirm \
         `git diff --stat crates/oracle-harness/tests/fixtures/constraints/` \
         is empty (byte-idempotent real-binary dump)."
    );
    Ok(())
}
