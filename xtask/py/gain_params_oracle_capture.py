...
holds (verified: `threshold=2.5000000000000004` for all three goldens, the
`offset==1` identity-binning convention).

This corpus does NOT demonstrate a smoothing-induced argmax change: at
`path_smooth=2.0` the candidate at threshold=1.5 (left={bin0,bin1}) and the
candidate at threshold=2.5 (left={bin0,bin1,bin2}) sum to a BIT-EXACTLY EQUAL
smoothed gain (their PER-SIDE outputs/gains differ, but the total does not),
so the REVERSE scan's strict `>` tie-break keeps whichever is visited first —
2.5, since REVERSE visits high-to-low. An earlier version of this script
claimed threshold=1.5000000000000002 for the path_smooth golden; that value
does not reproduce against a real, unmodified lightgbm==4.6.0 (verified
2026-08-06 — this exact script, unmodified, asserts the discrepancy loudly).
It was corrected to 2.5000000000000004, matching every other golden here.
...
EXPECTED_THRESHOLD = {
    ...
    # NOT 1.5 as originally claimed: for THIS symmetric corpus the smoothed
    # gain at threshold=1.5 (left={bin0,bin1}) and threshold=2.5
    # (left={bin0,bin1,bin2}) are BIT-EXACTLY EQUAL (verified: both sum to
    # 0x4060940000000000 via the two-axis gain primitives, per-side outputs
    # differ but the total does not), so the REVERSE scan's strict `>`
    # tie-break keeps the FIRST-visited candidate (2.5, visited before 1.5 in
    # REVERSE's high-to-low t order) exactly as lgbm_compute::gain does.
    # Re-verified 2026-08-06 against a real, unmodified lightgbm==4.6.0 run of
    # this exact script: it also picks 2.5, not 1.5. The previously-committed
    # golden and this constant were wrong — this corpus does NOT demonstrate a
    # smoothing-induced argmax change after all.
    "path_smooth": 2.5000000000000004,
}
