//! `resident_pool` — the device-resident histogram-pool MIRROR DISCIPLINE.
//!
...
//!   exactly — no reimplemented LRU / tree-growth.
//!
...
/// The device-resident SIZE GATE. Below this row count the per-leaf resident chain's
/// extra GPU launches dominate (the workload is launch-bound, not compute-bound), so
/// the byte-unchanged host path is the net win; at or above it the resident path's
/// saved host read-back/re-upload pays off.
///
/// The resident path LOSES at small row counts (launch/overhead-bound) and only WINS
/// at large row counts (compute-bound, where the saved per-leaf host read-back/re-upload
/// pays off). The threshold below is set at the measured crossover so small+medium
/// workloads route to the host winner and large workloads route to the resident winner.
/// `LGBM_RESIDENT_FORCE` overrides it for benching either path.
pub const RESIDENT_MIN_NUM_DATA: i32 = 12_000;

/// CONSERVATIVE / FAIL-SAFE device-resident eligibility predicate.
///
...
///
/// SIZE GATE + runtime override. After the fail-safe correctness checks, `num_data
/// < `[`RESIDENT_MIN_NUM_DATA`] routes the host path (the resident chain's extra
/// per-leaf launches lose on launch-bound tiny workloads). The size gate is a PERF
/// KNOB, not a correctness gate — both paths are proven equivalent (resident == host
/// tree), so routing either way grows the same tree within the ~1e-6 f32 contract.
///
...
pub fn resident_eligible(
    ...
    // ---- Runtime override THEN the num_data size gate ----
...
/// The FUSED directly-built-leaf SIZE GATE. The fused
...
/// the launch-bound regime in the SMALL/MEDIUM band (`num_data < `[`RESIDENT_MIN_NUM_DATA`])
/// — exactly where the 3-launch resident chain LOSES to the host path — to ask whether
/// collapsing to ONE launch flips that.
///
...
/// built leaves). But the NET wall-clock is FLAT-to-WORSE at every measured band: the
/// single-owner SEQUENTIAL f64 build the fused kernel uses (to stay bit-exact) replaces
/// the resident chain's PARALLEL f32-atomic build, and the sequential per-feature fold
/// costs MORE than the launches/leaf it eliminates — the launch saving does not pay for
/// the lost build parallelism. NO net win materialized.
///
/// The result is reported FLAT/NEGATIVE and the fused kernel is GATED OFF by default.
/// `FUSED_MAX_NUM_DATA = -1` makes `num_data <= FUSED_MAX_NUM_DATA` false for every real
/// workload (`num_data >= 1`), so the fused path never auto-engages; the proven bit-exact
/// kernel + oracle + `LGBM_FUSED_FORCE` override stay landed for future use (e.g. a future
/// device that makes a sequential f64 fold cheap, or a fused-subtract follow-up).
pub const FUSED_MAX_NUM_DATA: i32 = -1;

/// CONSERVATIVE / FAIL-SAFE FUSED directly-built-leaf eligibility predicate.
///
...
pub fn fused_directly_built_eligible(
    ...
    // VERDICT: the fused path is FLAT-to-NEGATIVE at every measured band, so
    ...
    // `FUSED_MAX_NUM_DATA` doc comment for the rationale.
    num_data <= FUSED_MAX_NUM_DATA
}

/// The `LGBM_SIBLING_COPACK` co-pack override, mirroring the
