...
/// ONCE per train instead of per tree.
#[must_use]
fn build_grow_features(
...
impl<'b, B: Backend> SerialTreeLearner<'b, B> {
    ...
    /// [`lgbm_compute::add_prediction_to_score_on_device_resident`] over the returned
    ...
    /// `< num_bin`, root `sum_hessian > 0` (the `cnt_factor` guard).
    /// `na_as_missing` features are admitted and trained, not rejected. Backend
    /// failures wrap via `#[from]`.
    pub fn train(
        &mut self,
        gradients: &[f32],
        hessians: &[f32],
    ...
    fn train_inner(
        ...
        if self.on_device_eligible {
            ...
            if self.backend.wants_resident_bins() && !self.resident_bins_uploaded {
                ...
                let upload_bins: Vec<&BinColumn> =
                    self.features.iter().map(|f| &f.bins).collect();
            ...
            let grow_features_owned: Vec<lgbm_compute::GrowFeature>;
            let grow_features: &[lgbm_compute::GrowFeature] =
                if ondevice_growfeat_memo_enabled() {
                    if self.grow_features_cache.is_none() {
                        self.grow_features_cache =
                            Some(build_grow_features(&self.features, &self.cfg));
                    }
                    self.grow_features_cache.as_ref().unwrap()
                } else {
        ...
        // The per-CALL cheap check (O(num_features), trivial) stays unconditional:
        // `f.bins.len()` depends on `num_data` (which differs between the full corpus
        // and a bagging subset), so it must be re-checked every call.
        ...
        // via `BinIndexOutOfRange`. Do NOT remove or weaken this relocated
        // mitigation: memoizing it (once per feature set) is safe ONLY because the
        // columns are immutable and the flag re-arms on every reassignment.
        // Reading/setting `self.bins_validated` is a SEPARATE field from the local
        // `features` Arc clone (line ~860), so it is not a borrow conflict.
        if !self.bins_validated {
            for f in features.iter() {
            ...
            // reassignment re-arms this for the next distinct feature set.
            self.bins_validated = true;
        }
