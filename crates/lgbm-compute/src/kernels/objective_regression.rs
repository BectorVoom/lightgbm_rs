...
/// Resident-Handle grad/hess kernel (ODS-02x): the f32-OUTPUT variant of the §5.1
/// math. The per-row body is transcribed VERBATIM from [`grad_hess_body`] at `F =
/// f64` (the deterministic f64 fold, D-10) — only the STORE differs: each output is
/// written as `f32::cast_from(...)` directly (the device-side `f64-compute → f32-cast`
/// order, matching `get_gradients_on`'s host-side `as f32` bit-for-bit, D-07). The
/// score arrives as an already-resident `Array<f64>` Handle (no host upload) and the
/// grad/hess stay on device as f32 output Handles (no readback in the launcher) — the
/// drop-in shape [`crate::ResidentGradHess`] (f32 grad/hess) expects. `f64::new(x)` is
/// the same constructor `grad_hess_body::<f64>` expands to.
#[cube(launch_unchecked)]
#[allow(unused_assignments, clippy::too_many_arguments)]
fn grad_hess_resident_kernel_f32(
    ...
    if i < scores.len() {
        ...
        let mut g = f64::new(0.0);
        let mut h = f64::new(1.0);
        ...
        if objective_tag == TAG_L2 {
            ...
            g = select(diff >= f64::new(0.0), f64::new(1.0) - param, f64::new(0.0) - param);
            h = f64::new(1.0);
        } else if objective_tag == TAG_HUBER {
            let ad = diff.abs();
            g = select(ad <= param, diff, sign_f::<f64>(diff) * param);
            h = f64::new(1.0);
        } else if objective_tag == TAG_FAIR {
            let denom = diff.abs() + param;
            g = param * diff / denom;
        ...
        if use_weight {
            // f64 multiply THEN f32-cast — identical to the host path's
            // `(g * w) as f32` (f64 mul, then narrow).
            grad[i] = f32::cast_from(g * weights[i]);
            hess[i] = f32::cast_from(h * weights[i]);
        } else {
...
/// Resident-Handle grad/hess launcher (ODS-02x) — the device-in/device-out sibling of
/// [`get_gradients_on`]. Takes an already-resident `Array<f64>` score `Handle` (NEVER
/// a host slice — the driver keeps the score device-resident, so there is no
/// per-call `create_from_slice` of the whole score) and writes f32 grad/hess directly
/// into two freshly allocated device Handles which it returns WITHOUT reading back
/// (the caller — Plan 04's `gbdt.rs` grad/hess branch — decides when/if to read them,
/// or feeds them straight into the resident build). Only the small per-train-constant
/// `labels`/`weights` are uploaded here. `objective_tag`/`param` carry the same
/// meaning as [`get_gradients_on`]; `num_data` is explicit because the Handle carries
/// no length.
///
/// Bit-exact vs [`get_gradients_on`] for the same `(score, labels, weights)` (proven
/// by `get_gradients_resident_on_matches_host_slice_all_tags`): the kernel runs the
/// same f64 per-row math and the same `f64-compute → f32-cast` order, only moving the
/// cast onto the device (`f32::cast_from`) instead of the host (`as f32`).
///
/// # Errors
...
mod resident_parity {
    use super::{
        get_gradients_on, get_gradients_resident_on, TAG_FAIR, TAG_HUBER, TAG_L1, TAG_L2,
        TAG_POISSON, TAG_QUANTILE,
    };
    use crate::runtime::cpu_client;
    use cubecl::prelude::*;

    /// ODS-03: the resident-Handle launcher is BIT-EXACT (`to_bits()` element-wise) vs
    /// the existing host-slice [`get_gradients_on`] for every §5.1 objective tag and
    ...
    #[test]
    fn get_gradients_resident_on_matches_host_slice_all_tags() {
        let client = cpu_client();
        // Diffs spanning +, -, and exactly 0 exercise every Sign/select branch; the
        // small non-negative scores/labels keep Poisson's exp() well-conditioned.
        let scores: Vec<f64> = vec![0.5, -0.25, 1.0, 2.0, 0.0, 1.5, 0.375, 0.75];
        let labels: Vec<f32> = vec![0.5, 0.75, 0.0, 1.0, 0.0, 0.5, 1.5, 0.125];
        let weights: Vec<f32> = vec![1.0, 2.0, 0.5, 1.25, 3.0, 0.75, 1.0, 2.5];
        let n = scores.len();

        // `param` per objective (L2/L1 ignore it): quantile alpha (f32-rounded),
        // huber delta, fair c, poisson exp(max_delta_step). The SAME param is passed
        // to both launchers so any difference can only come from the plumbing.
        let cases = [
            (TAG_L2, 0.0f64),
            (TAG_L1, 0.0),
            (TAG_QUANTILE, f64::from(0.7f32)),
            (TAG_HUBER, 0.9),
            (TAG_FAIR, 1.3),
            (TAG_POISSON, 0.2f64.exp()),
        ];

        for &(tag, param) in &cases {
            for weights_opt in [None, Some(weights.as_slice())] {
                let (host_g, host_h) =
                    get_gradients_on(&client, tag, &scores, &labels, weights_opt, param)
                        .expect("host-slice launcher");
                ...
                assert_eq!(res_g.len(), n, "grad length tag={tag}");
                assert_eq!(res_h.len(), n, "hess length tag={tag}");
                let weighted = weights_opt.is_some();
                for i in 0..n {
                    assert_eq!(
                        res_g[i].to_bits(),
                        host_g[i].to_bits(),
                        "grad bit mismatch tag={tag} weighted={weighted} i={i} \
                         (resident={} host={})",
                        res_g[i],
                        host_g[i]
                    );
                    assert_eq!(
                        res_h[i].to_bits(),
                        host_h[i].to_bits(),
                        "hess bit mismatch tag={tag} weighted={weighted} i={i} \
                         (resident={} host={})",
                        res_h[i],
                        host_h[i]
                    );
