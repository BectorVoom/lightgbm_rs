...
/// and reads back only the 6-int child ranges; every histogram build reads its
/// rows from an OFFSET VIEW of the device perm (no per-build `create_from_slice`
/// row upload); the host perm is read back ONCE at the grow tail for the layout
/// rebuild. Bit-exact by construction: the mark reuses the single-source
/// `route_to_left`, the scatter reproduces `partition_leaf_stable`'s stable order,
/// and the u64 build is order-independent — the `partition_resident=` COUNTS
/// tripwire proves the arm actually ran.
#[must_use]
pub fn resident_perm_partition_enabled() -> bool {
    // Same-session A/B override (mirrors `ondevice_fused_partition_enabled`): the
    // env gate is read-once, so an in-process A/B harness flips this atomic instead.
    match PARTITION_RESIDENT_OVERRIDE.load(Ordering::Relaxed) {
        1 => return true,
        2 => return false,
        _ => {}
    }
...
/// Same-session A/B override for [`resident_perm_partition_enabled`].
/// 0 = unset (defer to `LGBM_PARTITION_RESIDENT`), 1 = force ON, 2 = force OFF.
static PARTITION_RESIDENT_OVERRIDE: AtomicU8 = AtomicU8::new(0);

/// Harness/test hook: force the resident-perm partition arm ON (`Some(true)`), OFF
/// (`Some(false)`), or defer to the `LGBM_PARTITION_RESIDENT` env gate (`None`).
/// Same contract as [`set_fused_partition_override`] (the env gate is read-once and
/// cannot be re-read mid-process; the per-split read is a `Relaxed` load).
pub fn set_partition_resident_override(v: Option<bool>) {
    let code = match v {
        None => 0,
        Some(true) => 1,
        Some(false) => 2,
    };
    PARTITION_RESIDENT_OVERRIDE.store(code, Ordering::Relaxed);
}

/// POSITIVE tripwire — bumped once per split partitioned on the RESIDENT-PERM arm
/// ([`resident_perm_partition_enabled`]). A nonzero `partition_resident=` in the
/// COUNTS ledger is the bench-protocol proof the code-under-test actually ran
/// (the spike092 lesson: never trust a wall delta without a counts confirmation).
/// Inert unless `LGBM_PHASE_PROF=="1"` (parity-neutral).
pub static ON_DEVICE_PARTITION_RESIDENT_CNT: AtomicU64 = AtomicU64::new(0);

/// Bump the resident-perm partition tripwire (see [`ON_DEVICE_PARTITION_RESIDENT_CNT`]).
#[inline]
fn bump_partition_resident() {
    if launch_prof_enabled() {
        ON_DEVICE_PARTITION_RESIDENT_CNT.fetch_add(1, Ordering::Relaxed);
    }
}

/// Swap the resident-perm partition tripwire to zero and return the prior value
/// (folded into the `phase_prof` COUNTS line, like the other on-device counters).
pub fn on_device_partition_resident_count_take() -> u64 {
    ...
    R: cubecl::Runtime,
{
    ...
    // ---- RESIDENT-PERM partition arm (opt-in, `LGBM_PARTITION_RESIDENT=1`).
    // Keep the row permutation resident for the WHOLE grow: partition each split's
    ...
    let (root_best, root_fpos) = {
        ...
        time_phase(&GROW_BUILD_NS, || -> Result<(), ComputeError> {
            if let Some((_, rp, grow_max_abs)) = &resident_perm {
                // RESIDENT-PERM arm: the root's rows are the identity-seeded device
                // perm itself (0..num_data in order — same ids, same order as the
                // host `&perm` iota below, and the u64 build is order-independent
                // anyway ⇒ bit-identical histogram).
                backend.build_resident_leaf_rows_handle(
                    client,
                    0,
                    &slot_off,
                    slot_len,
    ...
    // ==================== SPEC-DRGL-05: DEFERRED-SYNC ARM (opt-in) ====================
    // When `LGBM_GROW_DEFER_SYNC` is ON and we are on the resident-perm arm, the per-split
    // `read_split` is DEFERRED one iteration: split i's children build/subtract/scan run
    // WITHOUT the host knowing split_point(i) — via the build-LEFT fixed-grid build
    // (which=0) + LEFT/RIGHT device-`num_data` scans (which=0/1). The split point is read
    // at the TOP of iteration i+1 (Phase 4a: a separate read; Phase 4b fuses it with the
    // pick into ONE `client.read`), where split i's HOST bookkeeping — the two children's
    // row ranges + the tree mutation (both need split_point) — is applied. The LAST split's
    // deferred read folds into the grow tail. `.slot`/sums/depth are host-known at i in the
    // build-LEFT framing (LEFT=fresh slot, RIGHT=parent slot — no smaller_is_left). The tree
    // is byte-identical to the flag-OFF path: the u64 resident histogram is order-free
    // (build-LEFT-directly == subtract-derived) and the LEFT/RIGHT scans == the host scans.
    if grow_defer_sync_enabled() {
        if let Some((view, rp, grow_max_abs)) = resident_perm.as_ref() {
            let grow_max_abs = *grow_max_abs;
            // The previous split's deferred state (row-range + tree-mutation inputs, minus
            // the counts that need split_point). `None` for the root iteration.
            struct Deferred {
                split_idx: usize,
                new_left: i32,
                new_right: i32,
            ...
            // Apply split `d`'s deferred bookkeeping now that `sp` (= split_point) is known:
            // set the two children's resident row ranges + issue the (in-order) device tree
            // mutation with the real counts. Byte-identical to the flag-OFF eager path — only
            // WHEN the host reads split_point back changes.
            let apply_deferred = |leaves: &mut Vec<ResidentDriverLeaf>,
                                  tree: &mut DeviceCudaTree<R>,
                                  d: &Deferred,
                                  sp: usize|
             -> Result<(), ComputeError> {
                let left_count = sp as i32;
                let right_count = (d.p_count - sp) as i32;
                leaves[d.new_left as usize].row_begin = d.p_begin;
                leaves[d.new_left as usize].row_count = sp;
                leaves[d.new_right as usize].row_begin = d.p_begin + sp;
                leaves[d.new_right as usize].row_count = d.p_count - sp;
                let scalars = SplitScalars {
                    is_valid: true,
                    leaf_index: d.new_left,
                    gain: d.gain,
                    inner_feature_index: d.real_feat,
                    threshold: d.threshold,
                    default_left: d.default_left,
                    left_sum_gradients: d.l_sum_g,
                    left_sum_hessians: d.l_sum_h,
                    left_sum_gh_quant: 0,
                    left_count,
                    left_gain: 0.0,
                    left_value: d.l_out,
                    right_sum_gradients: d.r_sum_g,
                    right_sum_hessians: d.r_sum_h,
                    right_sum_gh_quant: 0,
                    right_count,
                    right_gain: 0.0,
                    right_value: d.r_out,
            ...
            for split_idx in 0..(num_leaves - 1) {
                ...
                let best_fpos = *real_feat_to_fpos.get(&best_real_feat).ok_or_else(|| {
                    ComputeError::Runtime {
                        detail: format!(
                            "grow deferred: picked real feature index {best_real_feat} has no \
                             feature-position (reverse-lookup miss)"
                        ),
                    }
                })?;
                let best = SplitInfo {
                ...
                let real_threshold =
                    *f.bin_upper_bound.get(best.threshold as usize).ok_or_else(|| {
                        ComputeError::Runtime {
                            detail: format!(
                                "grow deferred: split threshold bin {} out of range for feature \
                                 {} (len {})",
                                best.threshold,
                                f.real_feature_index,
                                f.bin_upper_bound.len()
                ...
                // ---- PARTITION (device; NO read_split — deferred to the next iteration) ----
                ...
                // ---- Pool slots (build-LEFT): LEFT → fresh slot, RIGHT → parent slot. No
                // smaller_is_left needed; role assignment is skipped on this arm. ----
                let child_depth = parent_depth + 1;
                let left_slot = next_slot;
                next_slot += 1;
                let right_slot = parent_slot;
                // Seed children: sums (export) + slot + depth known NOW; row ranges DEFERRED.
                {
                    let l = &mut leaves[best_leaf as usize];
                    l.sum_g = best.left_sum_gradient;
                    l.sum_h = best.left_sum_hessian;
                ...
                leaves.push(ResidentDriverLeaf {
                    row_begin: 0,
                    row_count: 0, // DEFERRED (set when this split's read_split lands next iter)
                    sum_g: best.right_sum_gradient,
                    sum_h: best.right_sum_hessian,
                    slot: right_slot,
                    best: SplitInfo::none(),
                ...
                // ---- BUILD-LEFT (device count) → subtract RIGHT → scan LEFT + RIGHT ----
                bump_launch();
                time_phase(&GROW_BUILD_NS, || -> Result<(), ComputeError> {
                ...
                time_phase(&GROW_SUBTRACT_NS, || -> Result<(), ComputeError> {
                    backend.subtract_resident(client, parent_slot, left_slot, right_slot, slot_len)?;
                    grow_drain(client);
                    Ok(())
                })?;
                // Scannability: only the DEPTH cap is host-decidable without split_point; the
                // size (`min_data`) + `sum_h<=0` gates fall out of the scan's own reject (an
                // unscannable child folds the no-split sentinel, which the pick never selects)
                // — tree-identical to the flag-OFF explicit gate.
                let depth_ok = !(max_depth > 0 && child_depth >= max_depth);
                if depth_ok {
                    bump_launch();
                    time_phase(&GROW_SCAN_NS, || -> Result<(), ComputeError> {
                        backend.scan_resident_leaf_into_frontier_devcount(
                            client, left_slot, slot_len, &feats, &real_feats, cfg,
                            best.left_sum_gradient, best.left_sum_hessian, &leaf_splits_dev,
                            split_idx as usize, 0, p_count as i32, &frontier, new_left as usize,
                        )?;
                        grow_drain(client);
                        Ok(())
                    })?;
                    ...
                    reduce_winner_into_frontier(
                        backend, client, &frontier, new_left as usize, &SplitInfo::none(), -1,
                    )?;
                }
                if depth_ok {
                    bump_launch();
                    time_phase(&GROW_SCAN_NS, || -> Result<(), ComputeError> {
                        backend.scan_resident_leaf_into_frontier_devcount(
                            client, right_slot, slot_len, &feats, &real_feats, cfg,
                            best.right_sum_gradient, best.right_sum_hessian, &leaf_splits_dev,
                            split_idx as usize, 1, p_count as i32, &frontier, new_right as usize,
                        )?;
                        grow_drain(client);
                        Ok(())
                    })?;
                    ...
                    reduce_winner_into_frontier(
                        backend, client, &frontier, new_right as usize, &SplitInfo::none(), -1,
                    )?;
                }
                ...
                deferred = Some(Deferred {
                    ...
                    p_begin,
                    p_count,
                    real_feat: f.real_feature_index,
                    real_threshold,
                    missing_code: missing_type_code,
                    gain: best.gain + cfg.min_gain_to_split,
                    threshold: best.threshold,
                    default_left: best.default_left,
                    l_sum_g: best.left_sum_gradient,
                    l_sum_h: best.left_sum_hessian,
                    r_sum_g: best.right_sum_gradient,
                    r_sum_h: best.right_sum_hessian,
                    l_out: best.left_output,
                    r_out: best.right_output,
                });
            }
            // ---- TAIL: apply the LAST split's deferred read, then read the perm + tree. ----
    ...
    for split_idx in 0..(num_leaves - 1) {
        ...
        let p_begin = leaves[best_leaf as usize].row_begin;
        let p_count = leaves[best_leaf as usize].row_count;
        // PARTITION: the whole per-split row-routing op. RESIDENT-PERM arm: the §9
        // mark→scan→scatter runs IN PLACE on the device perm (3 launches, no bins
        // gather, no rows crossing) and the host reads back only the 6-int child
        // ranges (the split-point scalar its bookkeeping needs — the same small
        // readback the reference CUDADataPartition::SplitInner performs). Legacy
        ...
        // Build the SMALLER child's resident histogram into `slot`: the
        // RESIDENT-PERM arm reads the rows from an offset VIEW of the device perm
        // (no host slice — the host perm is STALE on that arm; no per-build row
        // upload); the legacy arm keeps the borrowed host-perm slice. The f64-fused
        // hatch never coexists with the resident arm (gated at state creation), so
        // its `s_rows` slice below stays host-perm-backed and correct.
        let build_smaller = |slot: usize| -> Result<(), ComputeError> {
            if let Some((_, rp, grow_max_abs)) = &resident_perm {
                backend.build_resident_leaf_rows_handle(
                    client,
                    slot,
    ...
    let _tail_guard = PhaseGuard::new(&GROW_TAIL_NS);
    // RESIDENT-PERM arm: the authoritative permutation lives on device — read it
    // back ONCE (the single per-grow perm crossing) so the layout rebuild below
    // groups the same rows the device partition produced. Bit-exact: the device
    // partition is byte-equal to the host arm's stable partition, so this readback
    // yields the identical Vec the host arm maintained incrementally.
    if let Some((_, rp, _)) = &resident_perm {
        bump_sync(); // one blocking per-grow readback (the tail perm crossing)
        perm = rp.read_perm(client);
    }
    let host_tree = tree.to_host_tree(client);
    let final_leaves = host_tree.num_leaves as usize;
