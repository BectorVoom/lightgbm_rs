...
impl<R: cubecl::Runtime> DeviceCudaTree<R> {
    ...
    pub fn split_on_device(
        ...
        self.launch_split_kernel(client, leaf_index, num_leaves, real_feature_index, real_threshold, missing_type, scalars);
        self.num_leaves += 1;
        Ok(SplitResult {
            new_node_index,
    ...
    /// 28-02 (ODF-02/ODF-05/ODF-06, R3): the NO-READBACK device tree split. Performs the
    /// SAME §10 [`split_kernel`] mutation as [`split_on_device`], but the right child leaf
    /// id is SUPPLIED BY THE CALLER from the fixed grow schedule (the driver's leaf
    /// counter) INSTEAD OF being returned for the host to read — retiring R3 (the
    /// `grow_driver.rs:1931` `right_leaf_index` `bump_sync`).
    ///
    /// `right_leaf_index_expected` is the driver's `leaves.len()` (the next leaf slot). The
    /// WR-03 desync invariant — that the tree's next leaf index equals the schedule's next
    /// slot — is asserted HOST-SIDE without any readback: it is `self.num_leaves`, which the
    /// driver keeps in lockstep (the same invariant `split_on_device`'s caller checks after
    /// the fact, moved to a pre-condition here). Returns `()` — the tree mutates in place and
    /// the caller already knows every leaf id from the schedule.
    ///
    /// Same field fan-out / NaN→0 leaf coercion / decision-type bits as
    /// [`split_on_device`], so the mutated device tree is byte-identical (ODF-06). Static
    /// geometry only; runs BEFORE the partition (§1/§10 ordering, Pitfall 3).
    ///
    /// # Errors
    /// [`ComputeError::Runtime`] if `leaf_index` is out of range, the split would exceed
    /// `max_leaves`, or `right_leaf_index_expected != self.num_leaves` (a schedule/tree
    /// leaf-id desync — the WR-03 guard, asserted without a readback).
    #[allow(clippy::too_many_arguments)]
    pub fn split_on_device_scheduled(
        &mut self,
        client: &ComputeClient<R>,
        leaf_index: i32,
        right_leaf_index_expected: i32,
        real_feature_index: i32,
        real_threshold: f64,
        missing_type: i32,
        scalars: &SplitScalars,
        ...
        self.check_split(leaf_index)?;
        let num_leaves = self.num_leaves;
        // WR-03 (no readback): the schedule's right leaf id MUST equal the tree's next leaf
        // slot (`num_leaves`). This is the same desync guard `split_on_device`'s caller runs,
        // asserted here as a pre-condition WITHOUT reading `right_leaf_index` back (R3).
        if right_leaf_index_expected != num_leaves {
            return Err(ComputeError::Runtime {
                detail: format!(
                    "DeviceCudaTree::split_on_device_scheduled: schedule right_leaf_index {} \
                     must equal the tree's next leaf slot {num_leaves} (schedule/tree desync)",
                    right_leaf_index_expected
                ),
            });
        }
        self.launch_split_kernel(client, leaf_index, num_leaves, real_feature_index, real_threshold, missing_type, scalars);
    ...
    /// The shared §10 [`split_kernel`] launch — the 15-thread scalar fan-out that mutates the
    /// flat device tree for ONE numerical split. Called by both [`split_on_device`] (which
    /// returns the derived `right_leaf_index`) and [`split_on_device_scheduled`] (which takes
    /// it from the schedule, no readback). Identical geometry + field writes.
    #[allow(clippy::too_many_arguments)]
    fn launch_split_kernel(
        &self,
        client: &ComputeClient<R>,
        leaf_index: i32,
        num_leaves: i32,
        ...
        let d = &self.device;
        let n = self.max_leaves;
        // SAFETY: `new_node_index` (`num_leaves - 1`) and the right leaf slot (`num_leaves`)
        // are `< max_leaves` (checked by `check_split`); every array is sized `max_leaves`.
        // The 15-thread fan-out writes disjoint cells. cubecl unsafe confined here (CMP-01).
        unsafe {
            split_kernel::launch(
                client,
