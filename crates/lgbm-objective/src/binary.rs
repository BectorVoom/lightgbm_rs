...
impl Binary {
    ...
    pub fn get_gradients(
        ...
        for i in 0..n {
            let is_pos = label[i] > 0.0;
            let label_val: f64 = if is_pos { 1.0 } else { -1.0 };
            let response = -label_val * sigmoid / (1.0 + (label_val * sigmoid * score[i]).exp());
            let abs_response = response.abs();
            gradients[i] = (response * label_weight) as f32;
            hessians[i] = (abs_response * (sigmoid - abs_response) * label_weight) as f32;
        }
        Ok(())
    }
