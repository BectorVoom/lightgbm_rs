        ...
        match self.kind {
            XentropyKind::CrossEntropy => {
                ...
                for i in 0..n {
                    ...
                    if score[i] > -37.0 {
                        let exp_tmp = (-score[i]).exp();
                        gradients[i] = (((1.0 - lab) - lab * exp_tmp) / (1.0 + exp_tmp)) as f32;
                        hessians[i] = (exp_tmp / ((1.0 + exp_tmp) * (1.0 + exp_tmp))) as f32;
                    } else {
                        let exp_tmp = score[i].exp();
                        gradients[i] = (exp_tmp - lab) as f32;
                        hessians[i] = exp_tmp as f32;
                    }
                }
            }
            XentropyKind::CrossEntropyLambda => {
                ...
                for i in 0..n {
                    ...
                    gradients[i] = (z - lab) as f32;
                    hessians[i] = (z * (1.0 - z)) as f32;
                }
            }
        }
        Ok(())
