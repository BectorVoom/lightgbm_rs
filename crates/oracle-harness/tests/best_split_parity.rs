...
fn best_split_parity_stage3_export() {
    ...
    let out = find_best_from_all_splits_on(&client, &mut per_leaf, 0, 1, 2).unwrap().cells;
    ...
    let out2 = find_best_from_all_splits_on(&client, &mut per_leaf, 0, 1, 2).unwrap().cells;
    ...
    let o = find_best_from_all_splits_on(&client, &mut none, 0, 1, 2).unwrap().cells;
