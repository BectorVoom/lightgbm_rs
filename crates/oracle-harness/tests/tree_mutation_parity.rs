...
/// 28-02 (R3, ODF-02/ODF-05/ODF-06): the NO-READBACK scheduled device tree split
/// (`split_on_device_scheduled`) mutates the device tree byte-identically to
/// `split_on_device` across a MULTI-split fixture — the right child leaf id is supplied
/// from the fixed schedule (the driver's leaf counter) instead of read back (R3 retired).
/// Both trees are reconstructed and compared field-for-field on the cpu f64 anchor (D-12).
#[test]
fn device_tree_split_scheduled_matches_host_mutation() {
    let Some(text) = read_split() else { return };
    let client = cpu_client();

    // Collect the splittable winners into a multi-split schedule.
    let mut winners: Vec<(String, SplitScalars, f64)> = Vec::new();
    for_each_winner(&text, |name, w| {
        winners.push((name.to_string(), scalars_from(w, 3), 1.5f64));
    });
    assert!(winners.len() >= 2, "need >= 2 splittable SCASE for a multi-split fixture");

    // Grow a `max_leaves`-leaf tree with BOTH APIs, always splitting leaf 0 (the current
    // left child) so the schedule's right leaf id is deterministically `num_leaves`.
    let max_leaves = winners.len() + 1;
    let root_count = 1000i32;
    let real_feature = 7i32;
    let missing_type = 0i32;

    let mut tree_ref = DeviceCudaTree::new(&client, max_leaves, root_count).expect("ref tree");
    let mut tree_sched = DeviceCudaTree::new(&client, max_leaves, root_count).expect("sched tree");

    for (i, (name, scalars, real_threshold)) in winners.iter().enumerate() {
        // Reference: split_on_device returns the derived right_leaf_index.
        let result = tree_ref
            .split_on_device(&client, 0, real_feature, *real_threshold, missing_type, scalars)
            .unwrap_or_else(|e| panic!("case `{name}`: split_on_device errored: {e:?}"));
        let expected_right = (i + 1) as i32; // fixed schedule: next leaf slot.
        assert_eq!(
            result.right_leaf_index, expected_right,
            "case `{name}`: reference right_leaf_index == schedule slot"
        );

        // Scheduled: right leaf id supplied from the schedule, NO readback.
        tree_sched
            .split_on_device_scheduled(
                &client,
                0,
                expected_right,
                real_feature,
            ...
            .unwrap_or_else(|e| panic!("case `{name}`: split_on_device_scheduled errored: {e:?}"));
    }

    // The two mutated device trees are byte-identical (bit-exact structure, D-12).
    let host_ref = tree_ref.to_host_tree(&client);
    let host_sched = tree_sched.to_host_tree(&client);
    assert_host_tree_eq("scheduled-vs-reference", &host_sched, &host_ref);

    // A wrong schedule leaf id is rejected host-side WITHOUT a readback (WR-03 guard).
    let mut tree_bad = DeviceCudaTree::new(&client, max_leaves, root_count).expect("bad tree");
    let (_, scalars0, thr0) = &winners[0];
    let err = tree_bad
        .split_on_device_scheduled(&client, 0, 99, real_feature, *thr0, missing_type, scalars0)
        .expect_err("desync schedule id must be rejected");
    assert!(
        matches!(err, lgbm_compute::error::ComputeError::Runtime { .. }),
        "desync must surface a typed Runtime error, got {err:?}"
    );
}
