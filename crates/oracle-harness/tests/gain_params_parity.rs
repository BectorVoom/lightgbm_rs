...
//! scan-level gain dispatch that changes the winning threshold for that
//! golden — `1.5` vs the other two goldens' `2.5`).
//!
...
/// reproduce lib_lightgbm's smoothed leaf outputs AND the DIFFERENT winning
/// threshold (`1.5` vs the other two goldens' `2.5`) — proving the smoothing
/// is threaded into the scan's per-candidate gain, not merely the final leaf
/// output.
#[test]
