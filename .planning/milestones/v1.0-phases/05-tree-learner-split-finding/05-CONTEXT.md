...
- **D-01:** ~~**Extend the header-only C++ transcription harness** (P1–P4 precedent) to produce the reference tree + per-split snapshots.~~ **SUPERSEDED by D-08 (gap closure, 2026-06-06).** The transcription harness shared the port's offset/`--th` conventions, so it could not falsify the shared-convention BLOCKERS CR-01/CR-02 (05-VERIFICATION.md) — it validated the port against itself. _Original text (for history):_ the `xtask` capture harness transcribes the learner verbatim, emits goldens over fixed g/h + binned inputs, and commits them; chosen over vendoring `external_libs` to build real `lib_lightgbm` and over a transcribe-then-cross-check hybrid.
...
### Gap closure (2026-06-06 — from 05-VERIFICATION.md gaps_found, user-authorized)
- **D-08:** **Replace the transcription harness with a real `lib_lightgbm` 4.6 oracle for the learner goldens (overrides D-01).** Build real `lib_lightgbm` 4.6 deterministically (`deterministic=true`, `force_row_wise=true`, `num_threads=1`, fixed seed; vendor `external_libs` / use the untracked `LightGBM/` tree — still never `git add` `LightGBM/`) and dump its model text as the committed reference trees. The hand-transcription capture is no longer the authoritative learner oracle because it shared the port's broken offset/`--th` conventions and so validated the port against itself (CR-01/CR-02). Real reference trees are committed for both the spine corpus AND a `most_freq_bin > 0` corpus so the `offset==1` path is anchored bit-exact against the real binary. (User-authorized reversal of locked D-01; resolves the oracle conflict in the gap report's `human_verification`.)
- **D-09:** **Adopt the real-LightGBM `offset==1` + compacted-histogram convention end-to-end for `most_freq_bin==0` (fixes CR-01 + CR-02 root cause).** Derive `offset` from `most_freq_bin` in a SINGLE shared helper used by both the runtime learner and (where still relevant) the harness; document it as the one authoritative convention. The stored threshold (`split.rs`) and the data-partition routing boundary (`partition.rs` `--th`) must agree so train-time partition routing is identical to the serialized tree's `get_leaf` predict routing. This both fixes the CR-01 off-by-one and makes the `offset==1` scan+partition path actually exercised (closing CR-02's coverage gap). An oracle-INDEPENDENT train/predict routing self-consistency assertion (grown tree's `get_leaf` over every training row must reproduce the data-partition `leaf_count` exactly) is added to `learner_parity.rs` as the regression test that fails today. (User-authorized; supersedes the minimal local-consistency patch option in the gap report.)
