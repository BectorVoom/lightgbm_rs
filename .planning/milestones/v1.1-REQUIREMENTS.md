# Requirements Archive: v1.1 CUDA On-Device Training Backend
...
- [x] **ODL-01**: The shared device-primitive kernels every subsystem builds on are ported as reusable CubeCL kernels — block + multi-kernel global **prefix-sum** (inclusive/exclusive), **shuffle reductions** (sum/max/min, dot-product), **bitonic argsort** (single- and multi-block, index-only — never moves values), and **weighted/unweighted percentile** — each anchor-pinned where it carries numeric output. (§2.4; §17 "port first".)
- [x] **ODL-02**: A CubeCL-safe device **split-record** (a pre-allocated `CUDASplitInfo` analog — NO per-split in-kernel device allocation, which has no clean CubeCL analog) and a **`CUDARandom` LCG** whose stream is bit-identical to the host `Random` (extra-trees / sampling / per-item-rand parity). (§15.)
...
- [~] **ODL-21** *(PARTIAL / DEFERRED to v2 — contingency correctly resolved to WITHHOLD)*: The on-device learner becomes the **DEFAULT** CUDA tree-learner path — contingent on anchor-pinned parity (~1e-6) AND not-slower than the current host-CUDA path on the Kaggle A/B — with the host path retained as the `LGBM_CUDA_ON_DEVICE=0` off-switch fallback. **Outcome:** the real-discrete-NVIDIA A/B returned FAIL (launch-bound, ~23 min/100-tree arm), so per plan D-09 the behavior-changing default-on flip was intentionally WITHHELD; `on_device_default()` stays `false`, on-device ships opt-in via `LGBM_CUDA_ON_DEVICE=1`. The audit-before-wire gate worked as designed; the literal end-state (on-device is default) is unmet and carried to v2 pending the per-leaf launch-bound perf work. See `phases/23-.../23-VERIFICATION.md`.
...
| ODL-20 | Phase 23 | Complete (harness ran on real CUDA; A/B FAIL verdict definitive; exact per-shape numbers dropped by Kaggle output-cap — re-run to record) |
| ODL-21 | Phase 23 | Partial / Deferred to v2 (default-on flip WITHHELD by design on A/B FAIL contingency; on-device ships opt-in) |
| ODL-22 | Phase 22 | Complete |
...
**Final status at milestone close (2026-07-03):** 21/23 satisfied. 2 partial — both Phase 23, both intentional v2 deferrals: **ODL-20** (real-CUDA A/B harness ran, FAIL verdict definitive, exact per-shape numbers dropped by Kaggle's output-size cap — harness fixed to self-evidence on re-run) and **ODL-21** (on-device default-flip correctly WITHHELD on the A/B FAIL not-slower contingency — ships opt-in via `LGBM_CUDA_ON_DEVICE=1`). All 10 phases (14–23) verified `passed`/`gaps_found`; every backend byte-unchanged with the env unset (merge gate green). The primary v2 carry-forward is the on-device per-leaf launch-bound perf work that would let ODL-21 be legitimately achieved, plus wiring the 7 anchor-pinned-but-unwired kernels (ODL-03/04/05/06/07/08/17). See `v1.1-MILESTONE-AUDIT.md`.
