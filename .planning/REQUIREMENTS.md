...
- [x] **BST-05**: DART boosting (`drop_rate`, `max_drop`, `skip_drop`, `uniform_drop`, `xgboost_dart_mode`, `drop_seed`)
- [x] **BST-06**: Random Forest boosting (averaged trees, mandatory bagging, no shrinkage accumulation)
...
- [x] **PRD-04**: Feature contributions / TreeSHAP (`predict_contrib`) over full tree node/cover structure
- [x] **PRD-05**: Prediction early stopping (`pred_early_stop`, `_freq`, `_margin`)
...
| TRL-01 | Phase 5 | Complete (05-09, bit-exact vs real binary on BOTH corpora — final mfb leaf-0 2-ULP closed via real-binary FP trace) |
| TRL-02 | Phase 5 | In Progress (gap closure 05-07; WR-01/02) |
| TRL-03 | Phase 5 | Complete |
| TRL-04 | Phase 5 | Complete |
| TRL-05 | Phase 5 | Complete (05-09 — spine + mfb FULLY bit-exact vs real binary incl. node-2 leaf-0 value) |
