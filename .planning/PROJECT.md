...
- [ ] Full objective-function set — *core subset validated in Phase 6 (regression, regression_l1, binary, multiclass, multiclassova, custom); ranking/lambdarank + remaining objectives in Phase 7*
- [ ] Full metric set — *core subset validated in Phase 6 (l1/l2/rmse, binary_logloss/error, auc, multi_logloss); ndcg + remaining metrics in Phase 7*
...
- **Current Rust crate:** Cargo workspace underway. Phases 1–6 complete (6/8 phases, 75%). Phase 1 (oracle-contract foundations), Phase 2 (`crates/lgbm-dataset`: bit-exact BinMapper, dense/sparse columnar bin store, missing/categorical encoding, EFB, metadata, dense/CSR/CSC ingestion), Phase 3 (`crates/lgbm-model`: Tree + GBDT model-text I/O, `%g` formatter, predict parity), Phase 4 (`crates/lgbm-compute`: Backend trait + histogram/split/partition/subtract kernels, bit-exact on the cubecl-cpu f64 anchor, best-effort cubecl-hip f32 on gfx1100), Phase 5 (`crates/lgbm-treelearner`: histogram serial tree learner + split finding, bit-exact vs `lib_lightgbm` 4.6), and Phase 6 (`crates/lgbm-objective`, `crates/lgbm-metric`, `crates/lgbm-boosting`, `crates/lgbm`: GBDT spine + 6 core objectives + 7 metrics + bagging + early stopping + Rust-native train/predict API, ~1e-6 f32 oracle parity) — all backed by the `oracle-harness` + `xtask` C++ golden-capture pipeline. Next: Phase 7 (parity-completing variants — GOSS/DART/RF, categorical/EFB splits, ranking, remaining objectives/metrics, SHAP/monotone/refit).
...
---
*Last updated: 2026-06-07 — Phase 6 (GBDT spine + core objectives/metrics + Rust-native API) complete; first end-to-end ~1e-6 (f32) train→predict validated (6/8 phases, 75%)*
