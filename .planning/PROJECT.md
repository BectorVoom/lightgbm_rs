...
**v1.1 Phase 16 (On-Device Histogram Constructor) — complete 2026-07-01.** The hot-path histogram — build → fix → subtract — now runs on-device: the two-tier §13-geometry BUILD kernel (`construct_leaf_hist_partition_u64` + the `_GlobalMemory` large-bin spill twin `construct_leaf_hist_partition_global_u64`) accumulates in block-local `Atomic<u64>` fixed-point (S=2^30) with an order-independent cross-block atomic merge and a separate de-quant pass (`dequant_leaf_hist`) — no f64 and no `Atomic<i64>` in the per-row scatter (ODL-09); `HistArena` is the allocate-exactly-once slot pool with the explicit `hist_t**` handle-rotation (larger derived in-place in the parent buffer, smaller into a fresh slot, no alias); `fix_histogram_mfb` (most-freq-bin omit-and-repair in the de-quanted `hist_t` float domain, compact step dropped) + `subtract_histogram_on_device` (reuses `subtract_hist_kernel` verbatim through the arena rotation, build-synced-before-subtract) + the `construct_histogram_for_leaf` build→dequant→fix→rotate→subtract entry complete the subtraction trick (ODL-10). All behind the OFF-by-default `LGBM_CUDA_ON_DEVICE` seam (`on_device_growth_supported()` stays false); shipped per-feature LDS kernel byte-unchanged vs base, default-features merge gate green (845/0). Code review: 0 blockers; WR-01/02/03 (untested sparse/multi-partition build path) recorded as forward risks to close before the seam is enabled. DEF-16-OOS-02 logged (pre-existing, kernel-byte-unchanged, nondeterministic f32-atomic ROCm near-tie flake, def-f8u-01 class). 5 plans, ODL-09/ODL-10 validated.

**v1.1 Phase 21 (Harden the On-Device Driver — parity corpus + WR-01 confirmation + bookkeeping) — complete 2026-07-02.** A parity-hardening re-cut: the end-to-end on-device grow driver itself (ODL-18/ODL-19) was delivered + verified in Phase 20 (6/6) per D-01, so Phase 21 confirms and broadens rather than builds. Three targeted STRUCTURE parity gates were added (deep >2-simultaneously-live-leaf, no-split/single-leaf break path, and a min_sum_hessian-constrained case via the new additive `grow_tree_on_device_driver_with_cfg`), each cloning the Phase-20 structure gate and reusing the tie-aware cpu-f64 comparator verbatim — bit-exact STRUCTURE / `default_left`, leaves within ~1e-5, on the cubecl-cpu default merge-gate lane and non-vacuous under `LGBM_CUDA_ON_DEVICE=1`. WR-01 (`HistArena::swap` slot-aliasing) confirmed closed (fix landed Phase 18, `c9a7fd1`) with repro + exhaustion tests green and documented as unit-test-locked (the live driver keeps per-leaf `Vec<f64>` and never consumes `HistArena`). The `Backend::grow_tree_on_device` trait seam is byte-unchanged; env-unset workspace stays byte-clean green. D-05 bookkeeping reconciled (ODL-18/19 → Complete under Phase 20; new ODL-18H the Phase-21 requirement). Code review: 0 blockers, 1 warning (stale function name in `_with_cfg`'s two `ComputeError::Runtime` diagnostic strings), 2 info. 3 plans, ODL-18H validated.
