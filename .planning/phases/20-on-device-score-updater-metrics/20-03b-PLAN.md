...
<task type="auto">
  ...
  <acceptance_criteria>
    ...
    - `grep -n 'FeatureColumn\|LeafSplits\|HistogramPool' crates/lgbm-compute/src` returns nothing — the
      driver reproduces the host order with its OWN state, never naming a treelearner type (no cycle).
...
<task type="auto">
  ...
  <action>
    ...
    continuous-feature L2 tree through the REAL driver on the cubecl-cpu runtime (construct the backend
    that grows on-device on the cpu runtime per 20-03a's gated CpuBackend flip, or drive
    ...
    anchor is ALWAYS the cubecl-cpu f64 fold — NEVER a second GPU f32 path (D-07/def-f8u-01). Replace the
    Slice-0 `host_grow` stand-in so the cell exercises a GENUINE on-device tree. Assert structure fields
    bit-exact, leaf values within `ROCM_LEAF_VALUE_TOL`, and default_left flips accepted ONLY on a
    genuine f32-vs-f64 split_gain near-tie (identical threshold + identical child row-counts); a non-tie
    flip hard-fails. The cell MUST execute in the DEFAULT cpu build (`cargo test -p oracle-harness --test
    learner_parity on_device` runs >=1 test, NOT filtered to 0); if an additional rocm-hardware variant
    is added it goes in a SEPARATE `#[cfg(feature="rocm")]` cell run with `--features rocm` and is NOT the
    primary gate. Record the ODL-19 no-f64 review: a grep-based inspection over the driver's new
    build/partition kernels confirming no f64 per-row grow/build hot loop (f64 permitted only in the
    reference-blessed scalar/gain math) — capture the reviewed kernel list + finding in the SUMMARY.
    Keep the gate behind the env so a fresh `cargo test --workspace` (env unset) is unchanged.
  </action>
  <verify>
    <automated>LGBM_CUDA_ON_DEVICE=1 cargo test -p oracle-harness --test learner_parity on_device && cargo test --workspace</automated>
  </verify>
  <acceptance_criteria>
    - `LGBM_CUDA_ON_DEVICE=1 cargo test -p oracle-harness --test learner_parity on_device` runs >=1 test
      (NON-vacuous — not `#[cfg(feature="rocm")]`-filtered to 0) and passes; the on-device tree is
      STRUCTURE bit-exact to `cpu_anchor_tree` with leaf values within `ROCM_LEAF_VALUE_TOL`.
    - The test compares the on-device tree ONLY against the cpu f64 anchor (no GPU-vs-GPU); the
      `host_grow` stand-in no longer stands in for the on-device tree.
  ...
  <done>The STRUCTURE gate asserts a real on-device tree bit-exact to the cpu f64 anchor with tie-aware default_left, runs non-vacuously in the default cpu merge-gate lane, and the ODL-19 no-f64 review is captured; the env-unset merge gate is unchanged.</done>
</task>
...
- `LGBM_CUDA_ON_DEVICE=1 cargo test -p oracle-harness --test learner_parity on_device` green and NON-vacuous (>=1 test) in the default cpu build (STRUCTURE bit-exact vs cpu f64 anchor).
- No f64 per-row grow/build hot loop introduced (reviewed + recorded in SUMMARY); u64 fixed-point build retained.
- `grep 'FeatureColumn|LeafSplits|HistogramPool' crates/lgbm-compute/src` finds nothing (no crate cycle).
</verification>
...
- `crates/oracle-harness/tests/learner_parity.rs`: activated STRUCTURE gate cell running a REAL
  on-device tree through `assert_on_device_tree_matches_cpu_anchor` on the cpu f64 anchor
  (host_grow stand-in replaced); non-vacuous in the default cpu build.
