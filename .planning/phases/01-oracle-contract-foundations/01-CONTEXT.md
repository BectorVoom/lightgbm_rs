...
- **D-07:** Use a **small C++ harness program linking `lib_lightgbm`** (including `Random`/`BinMapper` headers) to emit the 100k-draw RNG sequence and per-stage intermediates as fixtures; use the `lightgbm` CLI for end-to-end train/predict goldens. (CLI alone cannot expose RNG draws or pre-final intermediates needed by SC#2 and ORA-03.)
